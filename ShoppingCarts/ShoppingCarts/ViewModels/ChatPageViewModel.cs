...
namespace ShoppingCarts.ViewModels
{
    public class ChatPageViewModel : INotifyPropertyChanged
    {
        ...
        {
            Messages.Insert(0, new Message() { Text = "Добрый день, как я могу вам помочь?" });
            ...
            OnSendCommand = new Command(() =>
            {
                if (!string.IsNullOrEmpty(TextToSend))
                {
                    ...
                    {
                        Messages.Insert(0, new Message() { Text = "Ответ на " + TextToSend, User = "UserTwo" });
                    }
                    else
                    {
                        DelayedMessages.Enqueue(new Message() { Text = "Ответ на " + TextToSend, User = "UserTwo" });
                        PendingMessageCount++;
                    }
